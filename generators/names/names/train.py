...
from data import CharacterNameDataset, Vocabulary, OneHot, Groups, Races, ToTensor
...
class RNNCellTrainer(Trainer):
    ...
    def init_dataset(self):
        return CharacterNameDataset(root_dir=self.root_dir,
                                       ...
                                       group_transform=Compose([self.groups, OneHot(self.groups.size), ToTensor()]),
    ...
    def init_model(self):
        model = RNNCellModel(input_size=self.vocab.size + self.races.size + self.groups.size,
...
class RNNLayerTrainer(Trainer):
    ...
    def init_model(self):
        model = RNNLayerModel(input_size=self.vocab.size + self.races.size + self.groups.size,
                              hidden_size=self.hidden_size,
                              output_size=self.vocab.size)
        model.to(self.device)
        return model
